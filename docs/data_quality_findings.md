...
calculated. Both are documented below because they were bigger sources of
...
- Manually downloaded CSV exports (15-minute interval) for 11 stations from
  ...
  covering the full available history (October 2022 to August 2026, coverage
  varies by station).
...
of the results below. They are worth documenting because they initially
looked like massive data quality problems and were not:
...
4. **Floating-point false positives in gap detection.** Using `JULIANDAY`
   ...
   computes as e.g. 15.00003 minutes due to floating-point rounding. This
   technically satisfies a `> 15 minutes` gap filter but is not a real gap.
...
|---|---|---|
| Altamont Creek at Bluebell Dr | 57.3% | Oct 2022 to Aug 2026 |
| Arroyo Valle at Pleasanton | 62.6% | Nov 2022 to Aug 2026 |
| Arroyo Las Positas at Northfront Road | 81.3% | Apr 2023 to Aug 2026 |
| Altamont Creek at Pasatiempo Street | 86.6% | May 2023 to Aug 2026 |
| Alamo Creek at Willow Drive near Dublin | 90.6% | Oct 2022 to Aug 2026 |
| Arroyo Las Positas at Livermore | 91.1% | Oct 2022 to Aug 2026 |
| Rain Gauge at Dyer Rd | 91.6% | Oct 2022 to Aug 2026 |
| Line G1 at Dublin Blvd (rain gauge) | 92.5% | Oct 2022 to Aug 2026 |
| Tassajara Creek below I-580 (rain gauge) | 93.6% | Oct 2022 to Aug 2026 |
| Line G3 at Fairlands Dr (rain gauge) | 94.5% | Jun 2023 to Aug 2026 |
| Dublin Creek at Interstate 680 | 95.5% | Nov 2025 to Aug 2026 (short/recent history) |
...
Pleasanton.** Both rank worst by completeness % and total missing hours, and
both had single outages exceeding 8,000 hours (roughly a year), suggesting
extended sensor or transmission failures rather than routine intermittent
gaps.
...
- **0.3%** (52 gaps) exceed a full day. These are the true sensor/system
  outages, and they drive the large "total missing hours" numbers above
  despite being a tiny fraction of gap count.

### Metric-level nulls (separate from timing gaps)

Beyond missing timestamps, some stations report on schedule but have a
metric column that's blank on every row. This means a specific sensor is
not installed or is not reporting; data was not lost in transit.
...
This reflects which physical sensors each station actually has. "Blank"
needs to be interpreted per-station, not treated as a uniform missing-data
signal.
...
- **Medium gaps (2 hrs to 1 day, 1.4%):** exclude from daily aggregates
...
- **Long outages (> 1 day, 0.3% of gaps but the majority of total missing
  ...
  and likely reflect real equipment failures. Flag them to a data consumer
  instead of smoothing them over with interpolation.
- **Metric-level nulls:** do not interpolate. A sensor that is always blank
  is a missing capability, not a timing gap. Flag it separately from
...
- Date ranges vary by station (some have ~4 years of history, others only a
  few months), so completeness % is not perfectly apples-to-apples across
